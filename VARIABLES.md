...
| Name | Required | Default | Description |
| ---- | -------- | ------- | ----------- |
| use_kms_provisioning_key | N | true | Provide the KMS key used to encrypt the credentials during provisioning |
| kms_key_id | if `use_kms_provisioning_key=true` | | The OCID for the encryption key used for the provided credentials. |
| kms_crypto_endpoint | `use_kms_provisioning_key=true` | | The crypto endpoint for the KMS vault. |
...
| create_private_subnet | N | false | Create a private subnet for the Essbase node. A bastion host will be also created. |
...
| idcs_client_secret | if `security_mode=idcs` and `use_kms_provisioning_key=false` | | The client secret for the IDCS application, encrypted with the provided KMS key. |
| idcs_client_secret_encrypted | if `security_mode=idcs` and `use_kms_provisioning_key=true` | | The client secret for the IDCS application, encrypted with the provided KMS key. |
...
| db_admin_password | if `use_kms_provisioning_key=true` | | The password for the database administrator. Use a password that starts with a letter, is between 12 and 30 characters long, contains at least one number, and at least one of the special characters (`$` `#` `_`). For example, `BEstr0ng_#12`. |
| db_admin_password_encrypted | if `use_kms_provisioning_key=false` | | The password for the database administrator, encrypted with the provided KMS key. Use a password that starts with a letter, is between 12 and 30 characters long, contains at least one number, and at least one of the special characters (`$` `#` `_`). For example, `BEstr0ng_#12`. |
