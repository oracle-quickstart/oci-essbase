...
#
# Catalog Bucket
# Stores Essbase catalog 
#
module "catalog-bucket" {
  source         = "./modules/bucket"
  count  = local.instance_catalog_storage == "Object Storage Bucket" ? 1 : 0

  compartment_id = data.oci_identity_compartment.compartment.id
  bucket_name    = "essbase_${local.instance_uuid_short}_catalog"
  freeform_tags  = local.freeform_tags
  defined_tags   = local.defined_tags
}
...
module "network" {
  ...
  count  = (!var.use_existing_vcn && !var.is_upgrade) ? 1 : 0
...
module "database" {
  ...
  count                = ((local.db_type == "adb" && var.existing_db_id == "") && !var.is_upgrade)? 1 : 0
  compartment_id       = data.oci_identity_compartment.compartment.id
  ...
  create_secure_db      = var.create_secure_db
  vcn_id                = join("", module.network[*].vcn_id)
  subnet_id             = join("", module.network[*].application_subnet_id)
  freeform_tags        = local.freeform_tags
  defined_tags         = local.defined_tags
}
...
module "essbase" {
  ...
  metadata_bucket = module.metadata-bucket[0]
  backup_bucket   = module.backup-bucket[0]
  ...
  catalog_storage       = var.instance_catalog_storage
...
module "load-balancer" {
  ...
  count  = (local.create_load_balancer && !var.is_upgrade) ? 1 : 0

  compartment_id      = data.oci_identity_compartment.compartment.id
  subnet_ids          = flatten([module.existing-network.*.load_balancer_subnet_ids, module.network.*.load_balancer_subnet_ids])
  shape               = var.load_balancer_shape
  backend_nodes       = [for i in module.essbase[0].nodes : { ip_address = i.private_ip, port = i.listen_port }]
  display_name_prefix = local.resource_name_prefix
  freeform_tags       = local.freeform_tags
  defined_tags        = local.defined_tags
...
module "upgrade" {
  ...
  compartment_ocid         = data.oci_identity_compartment.compartment.id
