...
data "template_cloudinit_config" "config" {
  ...
  # The script which will initialize the VM as the root user 
  # in the background while the server is coming up
  part {
    content_type = "text/x-shellscript"
    content      = "${file("${path.module}/scripts/prepare_vm.sh")}"
  }
}
...
resource "oci_core_instance" "essbase" {
  ...
  metadata = {
    ...
    config_volume_device = "${var.config_volume_device}"
...
resource "oci_core_volume_attachment" "essbase_config" {
  ...
  device          = "${var.config_volume_device}"
...
EOT
  ...
  #    on_failure = "continue"
  #    inline = <<EOT
  #sudo -u oracle /u01/vmtools/shutdown.sh
  #EOT
  #
  #  }
}
