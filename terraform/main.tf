...
locals {
  ...
  use_existing_vcn = "${(var.use_existing_vcn) ? 1 : 0}"
  use_existing_db  = "${(local.use_existing_vcn && var.use_existing_db) ? 1 : 0}"
  ...
  assign_public_ip = "${var.assign_public_ip && (var.use_existing_vcn || (!var.use_existing_vcn && !var.create_private_subnet)) ? 1 : 0}"

  create_load_balancer = "${var.create_load_balancer ? 1 : 0}"

  enable_bastion = "${!local.assign_public_ip ? 1 : 0}"
}
...
module "network" {
  ...
  compartment_id      = "${local.use_existing_vcn ? var.existing_vcn_compartment_id : var.compartment_ocid}"
...
module "load-balancer-subnet" {
  ...
  use_existing_subnet = "${local.use_existing_vcn}"
  existing_subnet_ids = "${compact(list(var.existing_load_balancer_subnet_id, var.existing_load_balancer_subnet_id_2))}"
  compartment_id      = "${local.use_existing_vcn ? var.existing_vcn_compartment_id : var.compartment_ocid}"
  vcn_id              = "${module.network.vcn_id}"
...
module "bastion-subnet" {
  ...
  enabled             = "${local.enable_bastion ? 1 : 0}"
  use_existing_subnet = "${local.use_existing_vcn}"
...
module "database" {
  ...
  compartment_id    = "${local.use_existing_db ? var.existing_db_compartment_id : var.compartment_ocid}"
  db_name           = "${local.generated_atp_db_name}"
  db_admin_username = "${var.db_admin_username}"
  db_admin_password = "${var.use_kms_provisioning_key ? base64decode(join("", data.oci_kms_decrypted_data.decrypted_db_admin_password.*.plaintext)) : var.db_admin_password}"
